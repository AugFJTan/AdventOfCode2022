...
// Use stack data structure with pop, push
...
fn move_crates(stacks: &mut Vec<Vec<char>>, data: String) {
	...
	for _ in 0..count {
		let crate_to_move: char = stacks[from as usize].pop().unwrap();
		stacks[to as usize].push(crate_to_move);
	}
}
...
fn main() -> io::Result<()> {
	...
	for stack in stacks {
		print!("{}", stack[stack.len()-1]);  // Answer: ZRLJGSCTR
	}
	println!();

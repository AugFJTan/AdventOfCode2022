// Number of assignment pairs that overlap
...
fn main() -> io::Result<()> {
	...
	for line in reader.lines() {
		...
		if (first_pair_start <= second_pair_start && first_pair_end >= second_pair_start) ||
		   (second_pair_start <= first_pair_start && second_pair_end >= first_pair_start) {
			overlaps += 1;
		}
	}
